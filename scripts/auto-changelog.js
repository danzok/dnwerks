...
// Parse current changelog entries from embedded data
const parseCurrentEntries = () => {
  const content = readCurrentChangelog();
  
  // Find the getChangelogData function
  const functionMatch = content.match(/const getChangelogData = \(\) => \{[\s\S]*?\}/s);
  if (!functionMatch) {
    console.error('Could not find getChangelogData function in the file');
    process.exit(1);
  }
  
  // Extract the return statement with changelog data
  const returnMatch = content.match(/return \[([\s\S]*?)\];/s);
  if (!returnMatch) {
    console.error('Could not find changelog return statement in the file');
    process.exit(1);
  }
  ...
  if (!entryMatches) {
    console.error('Could not find changelog entries in the file');
    process.exit(1);
  }
...
${Object.entries(newEntry.impact).map(([key, value]) => `        ${key}: ${value}`).join(',\n')}
  ...
  // Find where to insert the new entry (after first entry, before others)
...
const main = () => {
  ...
  switch (command) {
    case 'add':
      if (args.length < 3) {
        console.log('Usage: node auto-changelog.js add <title> <description> [status] [changes...] [impact...]');
        console.log('Example: node auto-changelog.js add "New Feature" "Implemented user authentication" completed "Added login page" "Enhanced security"');
        process.exit(1);
      }
    ...
    case 'update':
      if (args.length < 3) {
        console.log('Usage: node auto-changelog.js update <id> <key=value> [key=value...]');
        console.log('Example: node auto-changelog.js update phase-1 status=completed impact.codeReduction=50');
        process.exit(1);
      }
    ...
    default:
      ...
      console.log('  add <title> <description> [status] [changes...] [impact...]');
      console.log('  update <id> <key=value> [key=value...]');
      console.log('  list');
      console.log('');
      console.log('Examples:');
      console.log('  node auto-changelog.js add "Bug Fix" "Fixed critical memory leak" completed "Updated dependencies"');
      console.log('  node auto-changelog.js add "New Feature" "Added dark mode support" completed "Enhanced UI" --impact.userSatisfaction=95 --impact.performance=30');
      console.log('  node auto-changelog.js update phase-1 status=completed impact.codeReduction=50');
      process.exit(1);
  }
};
